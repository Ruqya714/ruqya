...
export default function HealerBookingsPage() {
  ...
  return (
    <div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border">
            {isLoading ? (
              ...
              <tr><td colSpan={5}><EmptyState icon={<CalendarCheck size={28} className="text-text-secondary" />} title="لا توجد حجوزات" /></td></tr>
            ) : (
              bookings.map((b) => (
      ...
      <Modal isOpen={!!selected} onClose={() => setSelected(null)} title="تفاصيل الحجز" size="lg">
        {selected && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div><p className="text-xs text-text-muted mb-1">الاسم</p><p className="font-medium">{selected.patient_name}</p></div>
              <div><p className="text-xs text-text-muted mb-1">الهاتف</p><p className="font-medium" dir="ltr">{selected.patient_phone}</p></div>
