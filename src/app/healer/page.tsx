...
export default function HealerDashboard() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-text-primary mb-2">لوحة المعالج</h1>
      <p className="text-text-secondary mb-8">مرحباً بك — نظرة عامة على حجوزاتك</p>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        {[
          { label: "حجوزات اليوم", value: stats.today, icon: <CalendarCheck size={22} />, color: "bg-primary/10 text-primary" },
          { label: "حجوزات قادمة", value: stats.upcoming, icon: <Clock size={22} />, color: "bg-amber-50 text-amber-600" },
          { label: "إجمالي حجوزاتي", value: stats.total, icon: <TrendingUp size={22} />, color: "bg-accent/10 text-accent-dark" },
        ].map((stat, i) => (
      ...
      <div className="bg-white rounded-xl border border-border">
        <div className="px-6 py-4 border-b border-border">
          <h2 className="font-semibold text-text-primary">آخر حجوزاتي</h2>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-gray-50/50">
                ...
                <th className="text-right px-4 py-3 text-xs font-semibold text-text-secondary hidden sm:table-cell">الخدمة</th>
            ...
            <tbody className="divide-y divide-border">
              {recentBookings.length === 0 ? (
                <tr><td colSpan={4} className="py-8 text-center text-text-secondary">لا توجد حجوزات</td></tr>
              ) : (
