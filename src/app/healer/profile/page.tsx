  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-text-primary">ملفي الشخصي</h1>
      ...
      <div className="max-w-2xl">
        <div className="bg-white rounded-xl border border-border p-6 space-y-5">
          ...
          <div className="flex items-center gap-4 pb-5 border-b border-border">
            ...
            <div>
              <p className="font-semibold text-lg text-text-primary">{form.full_name || "المعالج"}</p>
              <p className="text-sm text-text-secondary">{form.specialization || "غير محدد"}</p>
            </div>
          </div>
          ...
          <div>
            <label className="block text-sm font-medium mb-1.5">التخصص</label>
            <input value={form.specialization} onChange={(e) => setForm({ ...form, specialization: e.target.value })} className="w-full px-4 py-2.5 rounded-lg border border-border text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" placeholder="مثال: رقية شرعية، علاج روحي" />
          </div>

          <div>
            <label className="block text-sm font-medium mb-1.5">نبذة عني</label>
            <textarea rows={4} value={form.bio} onChange={(e) => setForm({ ...form, bio: e.target.value })} className="w-full px-4 py-2.5 rounded-lg border border-border text-sm resize-y focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" placeholder="اكتب نبذة مختصرة عن خبرتك..." />
          </div>
          ...
          <div className="flex items-center gap-3 pt-3 border-t border-border">
            ...
            <label htmlFor="available" className="text-sm font-medium">متاح لاستقبال حجوزات جديدة</label>
          </div>
        </div>
      </div>
    </div>
