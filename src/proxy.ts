...
export default async function proxy(request: NextRequest) {
  ...
  // 2. Auth Protections — only check auth for protected routes
  const isProtectedRoute = cleanPath.startsWith("/admin") || cleanPath.startsWith("/healer");
  const isLoginPage = cleanPath === "/login";

  // Skip auth check entirely for public pages (major speed boost)
  if (!isProtectedRoute && !isLoginPage) {
    // Public page — no auth needed, skip DB calls entirely
  } else {
    const {
      data: { user },
    ...
    if (isProtectedRoute) {
      ...
      // Single profile query for both admin and healer routes
      ...
      } else if (cleanPath.startsWith("/healer")) {
        if (!profile || !["healer", "admin"].includes(profile.role) || !profile.is_active) {
          const url = request.nextUrl.clone();
          url.pathname = "/login";
          return NextResponse.redirect(url);
        }
